...
var drawTable = function () {
    ...
    connection.query(query, function (err, res) {
        ...
        var idLength = tableMaker_1.getLengthOfLongestItem(res, "item_id", "ID".length);
        var productLength = tableMaker_1.getLengthOfLongestItem(res, "product_name", "PRODUCT".length);
        var priceLength = tableMaker_1.getLengthOfLongestItem(res, "price", "PRICE".length);
        var stockLength = tableMaker_1.getLengthOfLongestItem(res, "stock_quantity", "IN STOCK".length);
        var separator;
        var lenSeparator;
        ...
        var stock = "IN STOCK" + tableMaker_2.generatePadding(stockLength, "IN STOCK".length);
        var header = "";
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            res[i].stock_quantity += tableMaker_2.generatePadding(stockLength, res[i].stock_quantity.toString().length);
        }
        ...
        console.log(separator);
        for (var i = 0; i < res.length; i++) {
            var lineItem = "| " + res[i].item_id + " | " + res[i].product_name + " | " + res[i].price + " | " + res[i].stock_quantity + " |";
